...
function updateSaveButtonsState() {
    ...
    cards.forEach((card, index) => {
        ...
        // Get numbers from the card (parse from DOM or store in data attribute)
        // Since we didn't store raw numbers in DOM, we might need to reconstruct them
        // Or better, rely on the fact that we can't easily get numbers back from DOM without parsing
        // Let's try to parse from the ball elements
        const balls = card.querySelectorAll('.lotto-ball');
        const numbers = Array.from(balls).map(b => parseInt(b.textContent));

        // Check if this combination exists in savedCombinations
        // savedCombinations might be empty if not loaded yet, so we should ensure it's loaded?
        // But loadMySavedCombinations is async.
        // Let's just check against the current savedCombinations array.
...
async function loadStatistics() {
    try {
        ...
        if (data.success) {
            ...
            elements.totalDraws.textContent = `${stats.total_draws}회`;
        }
    } catch (error) {
...
async function saveCombination(numbers, btnElement) {
    ...
    try {
        const response = await fetch(`${API_URL}/combinations/save`, {
            method: 'POST',
            headers: {
...
async function loadMySavedCombinations() {
    ...
    try {
        const response = await fetch(`${API_URL}/combinations/my`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
...
async function deleteSelected() {
    ...
    try {
        const response = await fetch(`${API_URL}/combinations/delete-batch`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
...
async function checkResults() {
    ...
    try {
        const response = await fetch(`${API_URL}/combinations/check-results`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
